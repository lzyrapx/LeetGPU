...
__global__ void histogram_kernel(const int* input, int* histogram, int N, int num_bins) {
    ...
    for (int pos = bid * block_size + tid; pos < N; pos += total_threads) {
        ...
        for (int k = 0; k < LOAD_FACTOR; k++) {
            ...
            vals[k] = (idx < N) ? input[idx] : -1;  // -1 作为无效标记
        }

        // 更新共享内存直方图
        #pragma unroll
        for (int k = 0; k < LOAD_FACTOR; k++) {
            if (vals[k] >= 0 && vals[k] < num_bins) {
...
extern "C" void solve(const int* input, int* histogram, int N, int num_bins) {
    ...
    int num_blocks = min(max_blocks, (N + block_size * LOAD_FACTOR - 1) / (block_size * LOAD_FACTOR));

    size_t shared_mem = num_bins * sizeof(int);
    histogram_kernel<<<num_blocks, block_size, shared_mem>>>(input, histogram, N, num_bins);
